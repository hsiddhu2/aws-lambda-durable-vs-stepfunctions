...
# Script to approve ETL jobs
# Usage: ./approve_job.sh <job-id> [reviewer-name] [reason]

set -e

JOB_ID=$1
REVIEWER=${2:-"manual-reviewer"}
REASON=${3:-"Approved via script"}
ACTION="approve"
...
if [ -z "$JOB_ID" ]; then
    ...
    echo "  $0 etl-stepfn-20260222001234-data_0002.csv 'john-doe' 'Data looks good'"
    exit 1
fi
...
if echo "$RESPONSE" | jq -e '.message' > /dev/null 2>&1; then
    echo ""
    echo "✓ Job $JOB_ID ${ACTION}d successfully!"
else
    echo ""
